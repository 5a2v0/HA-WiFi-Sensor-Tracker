...
class WifiSensorTrackerConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            # Controllo manuale: almeno un sensore selezionato
        ...
        schema = vol.Schema(
            {
                vol.Required(
                    "home_wifi_ssid",
                    default=self.config_entry.data.get("home_wifi_ssid"),
                ): str,
                vol.Required(
                    "sensors",
                    default=self.config_entry.data.get("sensors", []),
                ): selector(
                    {
                        "entity": {
                            "domain": "sensor",
                ...
                vol.Optional(
                    "consider_home",
                    default=self.config_entry.data.get("consider_home", 180),
                ): int,
            }
        )
